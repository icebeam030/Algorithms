...
WeightedQuickUnion::WeightedQuickUnion(int n) : N(n)
{
    if (n <= 0) {
...
int WeightedQuickUnion::get_root(int i)
{
    if (i < 0 || i > N - 1) {
...
bool WeightedQuickUnion::is_connected(int a, int b)
{
...
void WeightedQuickUnion::connect(int a, int b)
{
    if (a < 0 || a > N - 1 || b < 0 || b > N - 1) {
        throw invalid_argument("UnionFind: argument should be within [0, N - 1]!");
    }
